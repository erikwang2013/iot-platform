...
- [x] **Step 1: 写失败测试（纯函数部分：validate_rule + TOPIC_EVENTS 常量）**
...
- [x] **Step 3: 实现 engine.rs（替换 Task 3 占位）**
...
- [x] **Step 4: 实现 ws.rs（/ws?token=<JWT>，sub → 租户）**
...
- [x] **Step 1: 写失败测试（webhook 载荷纯函数部分）**
...
- [x] **Step 3: 写集成测试（真 MySQL + Kafka，#[ignore]）**
